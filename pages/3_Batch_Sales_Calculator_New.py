...
# NEW: Pallet-based quantity selection
st.subheader("📦 Pallet-Based Quantity Selection")
pallet_quantities = [2, 4, 6, 8, 10, 15, 20]  # Number of pallets

st.write(f"**Current product:** `{product}` with `{boxes_per_pallet}` boxes per pallet")

# Show what these pallet quantities translate to in boxes
pallet_info_data = []
for pallet_qty in pallet_quantities:
    box_qty = pallet_qty * boxes_per_pallet
    pallet_info_data.append({
        "Pallets": pallet_qty,
        "Boxes": box_qty,
        "Total Weight (approx)": f"{pallet_qty * weight_per_pallet_kg:.1f} kg"
    })

pallet_info_df = pd.DataFrame(pallet_info_data)
st.dataframe(pallet_info_df, use_container_width=True)

# Calculate batch quantities based on pallets
batch_quantities = [pallets * boxes_per_pallet for pallets in pallet_quantities]

st.write(f"**Calculation will be performed for:** {', '.join([f'{pallets}P ({pallets * boxes_per_pallet}B)' for pallets in pallet_quantities])}")
st.caption("P = Pallets, B = Boxes")
    ...
    if air_rates_df is not None and not air_rates_df.empty:
        ...
        for destination in air_destinations:
            ...
            for i, qty in enumerate(batch_quantities):
                # Get the corresponding pallet quantity
                pallet_qty = pallet_quantities[i]
                
                # Calculate total gross weight including pallet weight
                total_pallet_weight = pallet_qty * weight_per_pallet_kg
                ...
                # Store results for this pallet/box quantity combination
                column_name = f"{pallet_qty}P ({qty}B) - Sales Price/Box"
                dest_results[column_name] = current_sales_price_per_box
        ...
        if all_results:
            ...
            if push:
                ...
                webhook_url = "https://hook.eu2.make.com/wc1m7nnps682mj29xt1wuxx4wplerxyi"
